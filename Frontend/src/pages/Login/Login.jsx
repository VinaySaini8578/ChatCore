...
const Login = () => {
  ...
  // Auto-progress when OTP filled
  useEffect(() => {
    if (verificationCode.every(digit => digit !== '') && authStep === 'verification') {
      setErrors((prev) => ({ ...prev, otp: '' }));
      setAuthStep('registration');
    } else if (authStep === 'verification' && otpTouched) {
      // If not complete but user started typing, show hint
      setErrors((prev) => ({
        ...prev,
        otp: 'Please enter all 6 digits of the verification code.',
      }));
    }
  }, [verificationCode, authStep, setAuthStep, otpTouched]);
  ...
  const handleCompleteRegistration = async (e) => {
    ...
    const newErrors = {
      ...
      otp: validateOtp(verificationCode),
      email: '',
      gender: '',
    };
    setErrors(newErrors);
