...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = async (message) => {
      ...
      if (message.isGroup && message.conversationId) {
        ...
        // also refresh list so group moves to top
        try {
          const res = await userAPI.getCurrentChatters();
  ...
  // Add this function to refresh chatters when a message is sent
  const handleMessageSent = (userId, messageText) => {
    ...
    setChatters(prev => prev.map(chatter => {
      if ((chatter.isGroup && chatter._id === userId) || 
          (!chatter.isGroup && chatter._id === userId)) {
        return {
          ...chatter,
    ...
    setChatters(prev => {
      const updatedChatter = prev.find(c => 
        (c.isGroup && c._id === userId) || (!c.isGroup && c._id === userId)
      );
      
      if (updatedChatter) {
        const filteredChatters = prev.filter(c => 
          !((c.isGroup && c._id === userId) || (!c.isGroup && c._id === userId))
        );
        return [updatedChatter, ...filteredChatters];
      }
      return prev;
    });
  };
  ...
  return (
    <div className={styles.chatContainer}>
      ...
      <ArchivedPanel open={openArchived} onClose={() => { setOpenArchived(false); fetchChatters(); }} />
